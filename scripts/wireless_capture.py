...
def check_interface_for_monitor(iface: str) -> tuple[str, dict | None]:
    ...
    """
    interfaces = [i["name"] for i in get_all_interfaces_info()]
...
def run_capture_loop(
    ...
    if bssids:
        cmd.extend(["--bssid", bssids[0]])  # airodump-ng supports a single --bssid filter
        if len(bssids) > 1:
            info(f"Capturing {len(bssids)} APs — airodump-ng filters by first BSSID ({bssids[0]})")
...
def _try_pmkid_fallback(targets: list[dict], iface: str, args) -> None:
    """Attempt passive PMKID capture when airodump-ng got zero handshakes.

    Checks for hcxdumptool/hcxpcapngtool availability and chains into
    wireless_pmkid. Returns silently if tools are unavailable or if
    capture succeeds/fails — all reporting is done inline.
    """
    import shutil as _shutil

    hcxdump = _shutil.which("hcxdumptool")
    hcxconv = _shutil.which("hcxpcapngtool")

    if not hcxdump or not hcxconv:
        info("PMKID fallback unavailable (hcxdumptool/hcxtools not installed)")
        info("  Install: sudo pacman -S hcxdumptool hcxtools")
        return

    print(f"\n{YELLOW}No handshakes captured — falling back to passive PMKID capture{RESET}")
    info(f"  This does not require client deauthentication and works even")
    info(f"  when WPA3 Transition Mode or PMF blocks active capture.")

    from scripts.wireless_pmkid import pmkid_capture_targets
    ...
    print(f"\n{BOLD}{CYAN}═══ PMKID Fallback Complete ═══{RESET}")
    if result["success"]:
        status("PMKID capture file saved", True)
        info(f"  {result['pmkid_capture_file']}")
        if result.get("hashcat_file"):
            status("Hashcat format converted", True)
            info(f"  {result['hashcat_file']}")
        else:
            warn("No PMKID hashes extracted — AP may not respond to PMKID requests")
    else:
        status(result.get("error", "PMKID capture failed"), False)
...
def main() -> None:
    ...
    # ── Resolve interface (external USB adapter only) ──
    iface = args.interface
    if iface is None:
        best = require_external_adapter()
        iface = best["name"]
    else:
        # Validate that an explicit --interface choice is external
        external_names = [a["name"] for a in get_external_adapters()]
        if iface not in external_names:
            print(f"{RED}Interface '{iface}' is not an external USB adapter.{RESET}", file=sys.stderr)
            print(f"External adapters found: {external_names if external_names else 'none'}", file=sys.stderr)
            print("Internal WiFi is never used for wireless operations.", file=sys.stderr)
            sys.exit(1)
    ...
    # ── Auto-fallback to PMKID capture when no handshakes ──
    if handshake_count == 0:
        _try_pmkid_fallback(targets, iface, args)
