...
def find_system_wordlist() -> str | None:
    ...
    for path in candidates:
        ...
        if p.exists():
            return str(p)
    wz = Path("/usr/share/wordlists/rockyou.txt.gz")
    if wz.exists():
        return str(wz)
    return None
...
def run_aircrack(cap_file: str, wordlist: str, timeout: int) -> dict:
    ...
    for line in stdout.splitlines():
        line = line.strip()
        if line and not line.startswith(" ") and not line.startswith("Air"):
            psk = line
            success = True
            break
...
def main():
    ...
    parser.add_argument(
        "--backend", choices=["aircrack", "hashcat", "auto"],
    ...
    save_result(result, str(cap_path), wordlist, backend_name, output_dir)
