...
def classify_host(host: dict) -> dict:
    ...
    if role == "unknown" and mac_vendor:
        vendor = mac_vendor
        # Infer role from vendor
        if mac_vendor in ("Hikvision", "Dahua", "Axis"):
            role = "nvr-camera"
        elif mac_vendor in ("TP-Link",):
            role = "network-device"
        elif mac_vendor in ("Ubiquiti",):
            role = "wifi-controller"
        elif mac_vendor in ("Synology", "QNAP"):
            role = "storage-nas"
        elif mac_vendor in ("Amazon", "Google"):
            role = "iot-smart"
        elif mac_vendor in ("Philips",):
            role = "iot-lighting"
        elif mac_vendor in ("Raspberry Pi",):
            role = "iot-dev-board"
