...
# ── Test: detect_hash_type() ──


class TestDetectHashType:
    """Tests for hashid auto-detection."""

    def test_detects_wpa_hash_type(self):
        """hashid detects WPA handshake as mode 22000."""
        from scripts.wireless_crack import detect_hash_type

        with patch("scripts.wireless_crack.shutil.which", return_value="/usr/bin/hashid"), \
             patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="Analyzing 'test.cap'\n"
                       "  WPA PMKID [hashcat: 22000]\n"
                       "  WPA-EAPOL-PMK [hashcat: 22000]\n",
            )
            result = detect_hash_type("test.cap")

        assert result == ["22000"]

    def test_returns_empty_when_hashid_missing(self):
        """Returns empty list when hashid not installed."""
        from scripts.wireless_crack import detect_hash_type
    ...
    def test_returns_empty_on_failure(self):
        """Returns empty list when hashid fails."""
        from scripts.wireless_crack import detect_hash_type
...
class TestRunJohn:
    """Tests for john the ripper backend."""

    def test_launches_john_with_correct_args(self):
        """john is called with wordlist and capture file."""
        from scripts.wireless_crack import run_john

        with patch("scripts.wireless_crack.shutil.which", return_value="/usr/bin/john"), \
             patch("subprocess.run") as mock_run, \
             patch("pathlib.Path.exists", return_value=True), \
             patch("pathlib.Path.read_text", return_value=""), \
             patch("shutil.rmtree"):
            mock_run.return_value = MagicMock(returncode=0)
            result = run_john("/tmp/test.cap", "/tmp/words.txt", timeout=60)

        mock_run.assert_called()
        args = " ".join(str(a) for a in mock_run.call_args[0][0])
        assert "john" in args
        assert "test.cap" in args
        assert "words.txt" in args
        assert "--wordlist" in args

    def test_returns_psk_when_cracked(self):
        """Returns success with PSK when john cracks the hash."""
        from scripts.wireless_crack import run_john
        ...
        with patch("scripts.wireless_crack.shutil.which", return_value="/usr/bin/john"), \
             ...
             patch("shutil.rmtree"):
            mock_run.return_value = MagicMock(returncode=0)
            mock_read.return_value = "password123\n"
            result = run_john("/tmp/test.cap", "/tmp/words.txt", timeout=60)

        assert result["success"] is True
        assert result["psk"] == "password123"

    def test_returns_failure_when_no_crack(self):
        """Returns failure when john doesn't find the PSK."""
        from scripts.wireless_crack import run_john
    ...
    def test_returns_error_when_john_missing(self):
        """Returns error when john is not installed."""
        from scripts.wireless_crack import run_john

        with patch("scripts.wireless_crack.shutil.which", return_value=None):
            result = run_john("/tmp/test.cap", "/tmp/words.txt", timeout=60)
...
class TestBackendDispatch:
    """Tests for john integration into crack() dispatcher."""

    def test_crack_supports_john_backend(self):
        """crack() dispatches to john when --backend john."""
        from scripts.wireless_crack import crack

        with patch("scripts.wireless_crack.run_john") as mock_john, \
             patch("pathlib.Path.exists", return_value=True):
            mock_john.return_value = {"success": True, "psk": "cracked"}
            result = crack("/tmp/test.cap", "/tmp/words.txt", "john", 60)

        mock_john.assert_called_once()
        assert result["success"] is True
        assert result["psk"] == "cracked"

    def test_crack_auto_falls_back(self):
        """--backend auto tries aircrack -> hashcat -> john (via crack_auto)."""
        from scripts.wireless_crack import crack_auto

        with patch("scripts.wireless_crack.run_aircrack") as mock_air, \
             patch("scripts.wireless_crack.run_hashcat") as mock_hc, \
             patch("scripts.wireless_crack.run_john") as mock_john, \
             patch("pathlib.Path.exists", return_value=True):
            mock_air.return_value = {"success": False}
            mock_hc.return_value = {"success": False}
            mock_john.return_value = {"success": False}

            result = crack_auto("/tmp/test.cap", "/tmp/words.txt", 60)
