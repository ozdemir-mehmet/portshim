...
class TestCheckInterface:
    ...
    def test_returns_scan_mode(self):
        ...
        with (
            patch("scripts.wireless_capture.get_all_interfaces_info",
...
class TestCLIIntegration:
    ...
    def test_dry_run_shows_plan(self, capsys, monkeypatch, sample_targets_file):
        ...
        with (
            ...
            patch("scripts.wireless_capture.require_external_adapter",
                  return_value={"name": "wlan0", "capabilities": {"monitor_mode": True, "packet_injection": True}}),
            patch("scripts.wireless_capture.check_interface_for_monitor",
                  return_value=("full", {"capabilities": {"monitor_mode": True}})),
        ):
            main()
...
# ── Test: PMKID auto-fallback ──


class TestPmkidFallback:
    """Tests for _try_pmkid_fallback auto-chaining from capture."""

    def test_returns_silently_when_tools_missing(self):
        """Returns without error when hcxdumptool/hcxpcapngtool not installed."""
        from scripts.wireless_capture import _try_pmkid_fallback

        with patch("shutil.which", return_value=None):
            # Should not raise
            _try_pmkid_fallback(
                targets=[{"bssid": "aa:bb:cc:dd:ee:ff", "channel": 1}],
                iface="wlan1",
                args=MagicMock(duration=30),
            )

    def test_chains_into_pmkid_capture_when_tools_available(self):
        """Calls pmkid_capture_targets when tools are installed."""
        from scripts.wireless_capture import _try_pmkid_fallback

        mock_args = MagicMock(duration=30)

        with (
            patch("shutil.which",
                  side_effect=lambda c: f"/usr/bin/{c}"),
            patch("scripts.wireless_pmkid.pmkid_capture_targets") as mock_pmkid,
            patch("scripts.wireless_capture.json.dumps", return_value="{}"),
        ):
            _try_pmkid_fallback(
                targets=[{"bssid": "aa:bb:cc:dd:ee:ff", "channel": 1}],
                iface="wlan1",
        ...
        call_kwargs = mock_pmkid.call_args[1]
        assert call_kwargs["iface"] == "wlan1"
