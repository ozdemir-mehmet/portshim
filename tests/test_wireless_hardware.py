...
# ---------------------------------------------------------------------------
# get_external_adapters / require_external_adapter
# ---------------------------------------------------------------------------

class TestExternalAdapterOnly:
    """Tests that external-only functions filter internal adapters."""

    EXTERNAL_USB = {
        "name": "wlan1",
        "type": "external",
        "capabilities": {"monitor_mode": True, "packet_injection": True,
                         "band_2ghz": True, "band_5ghz": True, "band_6ghz": False},
    }
    INTERNAL_PCI = {
        "name": "wlan0",
        "type": "internal",
        "capabilities": {"monitor_mode": True, "packet_injection": True,
                         "band_2ghz": True, "band_5ghz": True, "band_6ghz": True},
    }

    def test_get_external_adapters_filters_out_internal(self, monkeypatch):
        """get_external_adapters returns only external adapters."""
        from scripts.wireless_hardware import get_all_interfaces_info

        monkeypatch.setattr(
            "scripts.wireless_hardware.get_all_interfaces_info",
            lambda: [self.INTERNAL_PCI, self.EXTERNAL_USB],
        )

        from scripts.wireless_hardware import get_external_adapters
        result = get_external_adapters()
        assert len(result) == 1
        assert result[0]["name"] == "wlan1"

    def test_get_external_adapters_empty_when_no_external(self, monkeypatch):
        """Returns empty list when only internal adapters present."""
        monkeypatch.setattr(
            "scripts.wireless_hardware.get_all_interfaces_info",
            lambda: [self.INTERNAL_PCI],
        )
    ...
    def test_require_external_adapter_returns_best(self, monkeypatch):
        """require_external_adapter returns the best external adapter."""
        monkeypatch.setattr(
            "scripts.wireless_hardware.get_external_adapters",
            lambda: [self.EXTERNAL_USB],
        )

        from scripts.wireless_hardware import require_external_adapter
        result = require_external_adapter()
        assert result["name"] == "wlan1"

    def test_require_external_adapter_exits_when_none(self, monkeypatch):
        """require_external_adapter exits with clear error when no external adapter."""
        monkeypatch.setattr(
            "scripts.wireless_hardware.get_external_adapters",
            lambda: [],
        )
