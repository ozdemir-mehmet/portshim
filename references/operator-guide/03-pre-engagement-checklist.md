...
- [ ] **USB-to-Ethernet adapter** — if your laptop lacks a native RJ45 port. **Dell USB-C adapter (Realtek RTL8153 chipset) recommended.** $15. Uses the in-kernel `r8152` driver with full hardware offload.
  - **Alternative:** D-Link DUB-1312 (ASIX AX88179 on `ax88179_178a` driver) — good backup, but verify with `ethtool -i` that the driver is `ax88179_178a`, not `cdc_ncm`.
  - ⚠ **Avoid ASIX AX88179 adapters** — they default to the generic `cdc_ncm` driver, which disables hardware checksum, scatter-gather, and VLAN offload.
  - ⚠ **Do not scan a wired network over WiFi** — ARP-based host discovery will fail because the broadcast domains are separated. You must have a direct Ethernet connection.
  - ⚠ **Dock Ethernet** — if your USB-C dock has a built-in NIC, it's fine in a pinch but shares the USB bus with DisplayLink and other peripherals. A dedicated adapter on its own bus is preferred.
  - ✅ Run `ethtool -k <interface> | grep -E 'checksum|scatter|segmentation|vlan-offload'` — all six should report `on`. If any are `off [fixed]`, the driver is crippled.
