...
def install_system_packages(pkg_mgr: str, dry_run: bool = False, with_msf: bool = False):
    ...
    base_pkgs = ["nmap", "git", "go", "python3", "pip", "nodejs", "npm", "graphviz",
                  "hydra", "sshpass", "aircrack-ng"]
    ...
    if with_msf:
        ...
        if pkg_mgr == "apt":
            ...
            for aur in ["paru", "yay"]:
                if shutil.which(aur):
                    run_cmd(["sudo", aur, "-S", "--noconfirm", "metasploit"], dry_run)
                    break
            else:
...
def install_python_deps(dry_run: bool = False):
    ...
    run_cmd(["pip", "install"] + PIP_PACKAGES, dry_run)
...
def main():
    ...
    if pkg_mgr:
        ...
        print("  Packages: nmap, git, go, python3, pip, nodejs, npm, graphviz, hydra, sshpass, aircrack-ng")
        print("  Go tools: " + ", ".join(GO_TOOLS))
        print("  Python: " + ", ".join(PIP_PACKAGES))
        sys.exit(1)

    print("\n=== Phase 1: System Packages ===")
    print("  (includes: nmap, git, go, python, node, graphviz, hydra, sshpass, aircrack-ng)")
    if args.with_msf:
    ...
    print("  (includes: openpyxl, python-docx, python-pptx, paramiko)")
